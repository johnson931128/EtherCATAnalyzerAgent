...
SDO_QUERY_ANSWER_REQUIREMENTS = (
    "Answer requirements for an SDO object capture question:\n"
    "- Default language is Traditional Chinese (繁體中文), unless the user explicitly "
    "requests another language.\n"
    "- Answer the user's actual engineering question directly. Review all authoritative "
    "transactions before answering.\n"
    "- Cover every piece of information explicitly requested by the user and all matched "
    "transactions relevant to that request; do not silently omit a relevant transaction.\n"
    "- Choose the clearest presentation yourself. Prose, bullets, or a Markdown table "
    "are all appropriate; a table can help compare repeated transactions, but no format "
    "is mandatory.\n"
    "- Prefer compact presentation when multiple transactions have the same structure. "
    "Avoid redundant repetition and do not restate details merely because they appeared "
    "in a table or bullet list.\n"
    "- Do not explain protocol concepts the user did not ask about unless needed to avoid "
    "a misunderstanding. Keep the answer engineering-focused and reasonably concise.\n\n"
    "Semantic safety constraints:\n"
    "- EtherCAT outgoing/returning path roles are not CoE SDO request/response roles.\n"
    "- `request_returning` is the same SDO Request copied after traversing the Slave. It "
    "is never an SDO Response and never a second SDO Request.\n"
    "- FPWR and FPRD are EtherCAT commands. FPRD may carry a matching CoE SDO Response, "
    "but that does not make the transaction an SDO Upload/read operation.\n"
    "- WKC is EtherCAT Datagram processing/execution evidence. WKC=1 does not mean SDO "
    "success and WKC must not be used as the SDO completion decision.\n"
    "- Python's grouped transactions and evidence assessment are authoritative. Do not "
    "regroup, change pairing, invent frames or values, infer missing deterministic "
    "evidence, recompute evidence status or PASS/FAIL/INCONCLUSIVE, choose another "
    "capture, request arbitrary new TShark scans, or build filters.\n"
    "- If Python evidence is ambiguous or incomplete, report it as ambiguous or incomplete "
    "and do not fill in a pairing or missing value.\n\n"
    "Use only the supplied query_spec, evidence_plan, evidence_assessment, evidence_trace, "
    "and normalized transactions/frame records. You may explain their EtherCAT/CoE "
    "meaning, but Python controls correctness while you control communication."
)
